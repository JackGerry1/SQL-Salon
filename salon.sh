...
MAIN_MENU () {
  ...
  then
    ...
    then
      ...
      NAME_AVAIL=$($PSQL "SELECT name FROM services WHERE service_id = $SERVICE_ID_SELECTED ")
      if [[ -z $SERV_AVAIL ]]
      then
      ...
      CUSTOMER_ID=$($PSQL "SELECT customer_id FROM customers WHERE phone = '$CUSTOMER_PHONE'")
        ...
        then
          ...
          then
            echo -e "\nI have put you down for a $NAME_SERV appointment at $SERVICE_TIME, $(echo $CUSTOMER_NAME | sed -r 's/^ *| *$//g')."
          fi  
        fi    
      fi
    fi  
